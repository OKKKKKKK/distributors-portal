...
import { publishCustomerEvent } from "../services/kafkaService";
...
export const createCustomer = async (req: Request, res: Response) => {
  try {
    ...
    if (result?.acknowledged) {
      // Publish the customer creation event to Kafka
      ...
      res
        .status(500)
        .json({ code: 500, message: "Failed to create a new customer." });
    }
  } catch (error) {
...
export const deleteCustomerById = async (req: Request, res: Response) => {
  try {
    ...
    const result = await collections?.customers?.deleteOne({
      _id: new ObjectId(id),
    });
...
export const deleteAllCustomers = async (_req: Request, res: Response) => {
  try {
    ...
    const nothingDeleted = Object.values(totalDeleted).every(
      (count) => count === 0
    );
    ...
    if (nothingDeleted) {
      return res.status(404).json({
        code: 404,
        message:
          "No data found to delete in customers, customerProducts, or orders.",
      });
    }
